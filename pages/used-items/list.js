...
export default function UsedItemsList(props) {
  ...
  return (
    ...
    <section>
          ...
          <Filter></Filter>
            <p className='text-sm text-gray-500 py-3'>{t('total')} {props.countProducts} {t('usedFound')}</p>
          <div className='grid lg:grid-cols-[3fr_1fr] gap-4'>
            <div className=''>
                ...
                <div className="lg:flex justify-center' py-2" key={property._id} >
                  <Link href={'/used-items/'+ property._id}>
                  ...
                  <Image
                    ...
                    <div className="py-4 px-6 w-full">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div>
                        { property.isFeatured ? <p className="flex border ml-5 r-0 border-yellow-600 text-white bg-yellow-600 uppercase px-4 py-1 rounded-full text-[9px] tracking-wide">
...
export async function getServerSideProps({ query }) {
  ...
  const categories = await UsedItems.find().distinct('category');
  const cities = await UsedItems.find().distinct('city');
  const areas = await UsedItems.find().distinct('area');
  const productDocs = await UsedItems.find(
    {
      ...queryFilter,
      ...categoryFilter,
      ...cityFilter,
  ...
  const countProducts = await UsedItems.countDocuments({
    ...queryFilter,
    ...categoryFilter,
    ...cityFilter,
    ...areaFilter,
