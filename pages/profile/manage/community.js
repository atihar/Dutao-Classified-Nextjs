...
export default function ManageadForSale() {
 ...
 const deleteHandler = async (productId, productImages) => {
  ...
  return (
      <>
        <Header></Header>
        <div className='sm:max-w-screen-xl sm:w-screen mx-auto md:py-4 px-8 md:my-4 rounded-lg shadow '>
            ...
            <div className="grid lg:grid-cols-[1fr_1fr_4fr] gap-4">
                ...
                <div className="overflow-scroll lg:overflow-auto">
                  <ul className="inline-flex lg:block whitespace-nowrap lg:ml-0 text-base space-y-0 md:space-y-4">
                          <Link href={'/profile/manage/property-for-sale'}><li className="hover:bg-gray-100 py-2 px-4 rounded-lg">{t('propertyForRent')}</li></Link>
                          <Link href={'/profile/manage/property-for-rent'}><li className="hover:bg-gray-100  py-2 px-4 rounded-lg ">{t('propertyForSale')}</li></Link>
                ...
                {posts && posts.map((ad) => (
                 ...
                 <Image
                   ...
                   <div className="py-2 px-6 w-full">
                     ...
                     <p className="text-gray-700 pb-1 text-xs">{ad.category}</p>
                     <p className="text-sm font-bold text-red-600">AED {ad.price}</p>
 
                     <p className="text-gray-400 pt-1 text-xs">Last updated {moment(ad.createdAt).startOf('hour').fromNow()}</p>
                       <p className="mb-2 text-sm pt-1 text-gray-500">{ad.bedroom} Bed - {ad.bathroom} Baths - {ad.size}sqft</p>     
                   </div>
                 </div>
               </div>
                ))}
