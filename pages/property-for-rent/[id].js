...
export default function singlePropertySale({property}){
                ...
                <div className="max-w-screen-xl p-5 mx-auto sm:px-6 lg:px-8">
                    <div className="grid sm:grid-cols-[2fr_1fr] gap-4">
                        <div>
                            ...
                            <p className="font-bold py-3">{t('aed')} {property.price}</p>
                            <div className="flex space-x-10 text-lg">
                                <div>{property.bedroom} {t('bed')}</div>
                                <div>{property.bathroom} {t('bath')}</div>
                                <div>{property.size} {t('sqft')}</div>
                            </div>
                            ...
                            <p className="mt-5">{t('aboutProperty')}</p>
                            <p className="text-lg text-gray-500">{property.description}</p>
                            <h3 className="py-4">{t('amenities')}</h3>
                            ...
                            <h3 className="mt-10 py-4">{t('propertyInfo')}</h3>
                            <div className="overflow-hidden overflow-x-auto border border-gray-100 rounded">
                                <table className="min-w-full text-base divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        ...
                                        <td className="p-4 text-gray-700 whitespace-nowrap">{property.furnished ? t('yes') : t('no')}</td>
                                    </tr>

                                    <tr>
                                        <td className="p-4 font-medium whitespace-nowrap">{t('propertyFor')}</td>
                                        <td className="p-4 py-2 text-gray-700 whitespace-nowrap">{t('rent')}</td>
                                    </tr>

                                    <tr>
                                        <td className="p-4 font-medium whitespace-nowrap">{t('listedBy')}</td>
                                        <td className="p-4 text-gray-700 whitespace-nowrap">{t('dutao')}</td>
                                    </tr>


                                    <tr>
                                        <td className="p-4 font-medium whitespace-nowrap">{t('postedOn')}</td>
                        ...
                        <div>
                            <div className="p-10 border border-gray-200 grid rounded-lg">
                                ...
                                <p className="text-sm pb-5">{t('dedLicense')} : 213234</p>
  ...
  export async function getServerSideProps(req, res) {
    ...
    const data = await RentProperty.findById(req.query.id).lean();
    await db.disconnect();
