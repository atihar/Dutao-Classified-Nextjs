                  ...
                  <Image
                    ...
                    <div className="py-4 px-6 w-full">
                      <div className="grid grid-cols-[3fr-1fr] relative items-center justify-between mb-2">
                        ...
                        <div>
                        {property.isFeatured ? <p className="flex top-0 right-0 absolute border border-yellow-600 text-white bg-yellow-600 uppercase px-2 py-0.5 rounded-full text-[7px] tracking-wide">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" className="bi bi-star" viewBox="0 0 16 16">
                              <path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 
                              0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"/>
                            </svg><span className='pl-2'>{t('featured')}</span>
                            </p> : "" }
                            
                            {property.isPromoted ? <p className='text-[10px] text-gray-400 top-4 right-0 absolute px-2'>sponsored</p> : "" }
                            
                            {property.businessLogo ? <img className=' absolute bottom-0 right-0 w-16' src={`${property.businessLogo}`}/> : ""}
                        </div>       
                      </div>                         
                    </div>
                  </div>
...
export async function getServerSideProps({ query }) {
  ...
  const areas = await RentProperty.find().distinct('area');
  const productDocs = await RentProperty.find(
    {
      ...queryFilter,
      ...categoryFilter,
      ...priceFilter,
  ...
  const countProducts = await RentProperty.countDocuments({
    ...queryFilter,
    ...categoryFilter,
    ...priceFilter,
    ...sizeFilter,
