...
export default function verification() {
    ...
    useEffect(() => {
      if(token){
          axios.get(`/api/user/verification?continue_verification=${token}`,)
  ...
  return (
    ...
    <div className='max-w-screen-lg mx-auto p-10'>
      ...
      <div className='flex flex-col items-center bg-green-50 py-8 px-16 rounded-xl'>
        ...
        <h1>{message}</h1>
        <p className='text-base'>What's next? <Link href={'/login'}><span className='underline cursor-pointer'>continue</span></Link></p>
      </div>
    :
